    ...
    public class AMF0
    {
        public static List<AMFType> Decode(byte[] bytes)
        {
            ...
            while (bytes.Length > 0)
            {
                var type = typeAttributes.First(t =>
                    ((AMFTypeAttribute)t.GetCustomAttributes(typeof(AMFTypeAttribute), false)[0]).Type == bytes[0]);
        ...
        public static byte[] Serialize(List<AMFType> amf)
        {
            ...
            foreach (var message in amf)
            {
                ...
                var attr = message.GetType().GetCustomAttributes(false)[0] as AMFTypeAttribute;
                bytes.Add(attr.Type);
